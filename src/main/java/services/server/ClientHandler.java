	...
	public void run() {
		try {
			...
			case "GET_FILE_PATH" -> {
				...
				String response = getFilePath(Integer.parseInt(id));
				sendResponse(response);
			}
			case "GET_FOLDER_PATH" -> {
				String id = (String) receiveRequest();
				String response = getFolderPath(Integer.parseInt(id));
				sendResponse(response);
			}
			case "GET_NEW_FILE_PATH" -> {
				String id = (String) receiveRequest();
				String name = (String) receiveRequest();
				String response = getFileChanged(Integer.parseInt(id), name);
				sendResponse(response);
			}
			case "RENAME_FILE" -> {
      ...
      case "GET_ALL_DELETED_ITEM" -> {
			...
			case "UPLOAD_FOLDER" -> {
				...
				if (type_request.equals("folder")) {
					...
					PermissionService permissionService = new PermissionService();
					int permission_type = permissionService.getPermission(PermissionService.FOLDER_TYPE,
							currentFolderId);
...
//                receiveFileThread.start();
	...
	private String getFolderPathChanged(int folderId, String folderName) {
		FolderService folderService = new FolderService();
		String rs = folderService.getFolderPathChange(folderId, folderName);
		return rs;
	}
	...
	private boolean uploadFolder(String folderName, int ownerId, int parentId, int permission_type)
